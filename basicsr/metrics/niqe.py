...
@METRIC_REGISTRY.register()
def calculate_niqe_ir_sobel(img, crop_border, input_order='HW', **kwargs):
    """Calculate NIQE (Natural Image Quality Evaluator) metric for infrared images with Sobel features.

    Args:
        img (ndarray): Input infrared image whose quality needs to be computed.
            The input image must be a single channel image.
            The input_order of image can be 'HW'.
        crop_border (int): Cropped pixels in each edge of an image. These
            pixels are not involved in the metric calculation.
        input_order (str): Whether the input order is 'HW'. Default: 'HW'.
    ...
    # normalize pixel values to [0, 255] if necessary
    if img.max() > 255:
        img = img / img.max() * 255
